...
- API Key는 기본적으로 브라우저 탭의 메모리에만 유지되며 저장소·로그에 남지 않음. 원하면 키마다 **"이 브라우저에 저장"**을 켜서 이 기기 전용 암호화 키로 암호화된 형태로만 남겨 다음에 다시 입력하지 않게 할 수도 있음 (8장 참고). Gmail 연결 정보는 저장 대상이 아니며 항상 탭 메모리에만 유지됨
...
Vitest로 파일 파싱, Jev 응답 정규화, 정책(임계값) 판정, Gemini 프롬프트 규칙, 분석 결과 캐싱(동시성 경쟁 상태 포함), Gmail 자동 발송 안전장치, Gmail MIME 인코딩, API Key 기본 비저장 및 "이 브라우저에 저장" 옵트인 동작, API Key 암호화/복호화 왕복, CSV 한글 보존 등을 검증합니다.
...
### API Key 저장 방식 (기본값 vs "이 브라우저에 저장")

**기본값은 저장하지 않는 것**입니다. 아무것도 켜지 않으면:
...
매번 입력하기 번거롭다면, 각 키 입력란 아래의 **"이 브라우저에 저장"** 체크박스를 켤 수 있습니다.

- 켜면 이 기기·이 브라우저 프로필에서만 사용할 수 있는, 추출할 수 없는(non-extractable) 암호화 키(AES-GCM, IndexedDB에 보관)로 API Key를 암호화해 `localStorage`에 저장합니다. **평문으로 저장되지 않습니다.**
- 다음에 이 브라우저로 같은 사이트에 접속하면 자동으로 복호화되어 다시 입력할 필요가 없습니다.
- **무엇으로부터 보호되고, 무엇으로부터 보호되지 않는지 정확히 알아두세요.** 이 방식은 `localStorage`를 통째로 복사해 다른 기기로 가져가거나, 화면 공유 중 누군가 devtools를 얼핏 들여다보는 상황으로부터 평문 노출을 막아줍니다. 하지만 이 페이지에서 실행되는 악성 스크립트(XSS)나, **이 브라우저 프로필 자체에 접근할 수 있는 사람**(예: 같은 계정으로 로그인한 공용 컴퓨터)으로부터는 보호하지 못합니다 — 그 경우 누구나 이 앱과 똑같은 방식으로 복호화해 값을 읽을 수 있습니다.
- 설정 화면의 **"저장된 키 모두 지우기"** 버튼으로 언제든 삭제할 수 있고, 체크박스를 다시 끄면 그 즉시 해당 키의 저장된 값이 삭제됩니다.
- **본인만 사용하는 개인 기기에서만 켜고, 공용·공유 컴퓨터에서는 절대 켜지 마세요.**

공통 사항:

- 브라우저는 TypeSafe·Google API를 직접 호출하지 않고, Netlify Functions로 요청을 보낼 때만 키를 함께 전달합니다.
- Netlify Functions는 키를 사용해 외부 API를 호출한 뒤 **키를 저장하거나 로그에 남기지 않습니다.** (저장 여부와 무관하게 서버에는 항상 남지 않습니다)
- Gmail 연결(access token)은 저장 대상이 아니며 항상 탭 메모리에만 유지됩니다. Gmail access token은 수명이 짧아(약 1시간) 저장해 둘 실익이 적고, 세션마다 다시 연결하는 것이 표준적인 OAuth 방식이기도 합니다.
...
src/
  ...
  lib/
    ...
    secureCrypto.ts      AES-GCM 암호화/복호화 순수 로직
    secureKeyStorage.ts  "이 브라우저에 저장" 옵트인 시 사용하는 암호화 저장소 (IndexedDB 키 + localStorage)
...
- API Key "이 브라우저에 저장"은 브라우저 프로필 자체에 접근할 수 있는 사람으로부터는 값을 보호하지 못합니다(XSS나 공유 로그인 계정 등). 개인 전용 기기에서만 사용하세요. 또한 브라우저 데이터를 지우거나(사이트 데이터 삭제 포함) 시크릿/비공개 모드에서는 저장된 키가 유지되지 않습니다.
